...
ARCHIVO_CSV = CARPETA_BASE / "data" / "data-encuestas-martes.csv"
...
RESUMENES_IA = {
    "Contenido que les gustaría ver en clase": (
        "La mayoría quiere programas prácticos aplicados a su trabajo: automatizar "
        "tareas administrativas repetitivas, softwares de gestión para pymes, "
        "conciliaciones contables/bancarias y medición de métricas de un sector como "
        "Compras. También aparece el interés por actividades interactivas (como el "
        "Kahoot) y por resolver problemas parecidos a situaciones laborales reales."
    ),
    "Sugerencias para mejorar el ambiente en clase": (
        "Bastantes dicen que el ambiente ya está bien y no tienen sugerencias, y varios "
        "destacan la buena energía del profesor. Entre los comentarios puntuales piden ir "
        "más despacio y explicar desde lo más básico para quienes tienen poco o nulo "
        "conocimiento previo, hacer las clases más interactivas, y sumar una pequeña "
        "sinopsis de cada clase para poder ponerse al día si se la perdieron."
    ),
    "Ideas para incentivar el uso de cámaras": (
        "Varios no pueden o no quieren prender la cámara: falta de webcam en la PC, "
        "estar trabajando en el horario de clase, o tener que cuidar a un hijo/a. Como "
        "ideas para incentivarla aparecen hacer actividades o juegos, sacar fotos "
        "grupales, y una propuesta más firme de contar la asistencia solo si se prende "
        "la cámara al entrar (y volver a chequear después de cada recreo)."
    ),
    "Recomendaciones de IT (libros, videos, documentos)": (
        "La mayoría no tiene ninguna recomendación para compartir. Entre quienes sí "
        "mencionan algo aparecen los cursos de Lucas Dalto en YouTube (nombrado dos "
        "veces) y un artículo de LinkedIn sobre por qué se retrasan los proyectos SAP."
    ),
    "Recomendaciones de libros o películas": (
        "Predominan las series y películas: La Casa del Dragón, Silo, The Social "
        "Network, Volver al Futuro, Red Social, Game of Thrones. En libros aparecen "
        "Hábitos Atómicos, Padre Rico Padre Pobre, La Paciente Silenciosa, Strange "
        "Pictures, 1984, Sherlock Holmes, Crónicas Marcianas y El Hombre Ilustrado."
    ),
    "Pregunta abierta": (
        "Aparecen preguntas curiosas sobre la cursada (qué los hizo elegir programación, "
        "si van a ver otro lenguaje además de Python) junto con comentarios personales: "
        "agradecimiento y buena onda con el profesor, y un relato sobre lo difícil que es "
        "sostener la materia por segunda vez entre el trabajo y el cansancio, pidiendo "
        "algún consejo para encararla mejor."
    ),
}
...
def generar_html(respuestas, graficos):
    ...
    partes = [
        ...
        "<head>",
        '<meta charset="utf-8">',
        "<title>Encuesta social - C Datos (Martes)</title>",
        "<style>",
        ...
        "</head>",
        "<body>",
        f"<h1>Encuesta social - C Datos (Martes)</h1>",
